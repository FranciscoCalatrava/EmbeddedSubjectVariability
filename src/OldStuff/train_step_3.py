...
class Trainer_3():
    ...
    def FADA_g(self, output, targets):
        Discriminator = targets.to(self.device).float()
        ...
        # print(otuput.shape)
        # print(Discriminator.shape)
        return self.criterion_discriminator(sigmoid(output[labels_1]), torch.unsqueeze(Discriminator[labels_1],1))
    ...
    def train_one_epoch(self):
        ...
        for batch_discriminator, batch_classifier in zip(self.train_dataloader_discriminator, self.train_dataloader_classifier):
            ...
            discriminator_labels = targets.to(self.device).float()
            ...
            predictions_discriminator = (sigmoid(outputs_discriminator_3_1) > 0.5).int()
            ...
            loss = self.criterion_discriminator(sigmoid(outputs_discriminator_3_2), torch.unsqueeze(discriminator_labels,1))
            ...
            predictions_discriminator = (sigmoid(outputs_discriminator_3_2) > 0.5).int()
    ...
    def validation_discriminator(self):
        ...
        with torch.no_grad():
            for discriminator_batch in self.val_dataloader_discriminator:
                ...
                loss_discriminator = self.criterion_discriminator(sigmoid(outputs_discriminator),torch.unsqueeze(discriminator_labels,1))
                ...
                predictions_discriminator = (sigmoid(outputs_discriminator) > 0.5).int()
