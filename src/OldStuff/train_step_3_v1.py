'''
In this version we will try the gradient reversal version of the adversarial learning.
'''
...
class Discriminator_1(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.model = nn.Sequential(OrderedDict([
            ('Gradient_Reversal', GradientReversal(alpha=1.0)),
...
class Trainer_3():
    def __init__(self, encoder, decoder, discriminator, classifier, train_dataloader_discriminator, val_dataloader_discriminator, train_dataloader_classifier, val_dataloader_classifier, device, optimizer, criterion, weight, dataset, experiment, seed,id) -> None:
    ...
    def train_one_epoch(self):
        ...
        for batch_discriminator, batch_classifier in zip(self.train_dataloader_discriminator, self.train_dataloader_classifier):
            ...
            loss_discriminator_3_1 = 0.5*self.criterion_discriminator(outputs_discriminator_3_1, discriminator_labels)
            #self.FADA_g(outputs_discriminator_3_1, targets)
            #self.criterion_discriminator(outputs_discriminator_3_1, discriminator_labels)
            # self.FADA_g(outputs_discriminator_3_1, targets)
            loss_classifier_3_1 = 0.25*self.criterion_classifier(outputs_classifier_3_1, targets_classifier)
            loss_reconstruction_0 = 0.25*self.criterion_reconstruction(outputs_decoder_0_3_1, torch.unsqueeze(inputs[0], 2))
            loss_reconstruction_1 = 0.25*self.criterion_reconstruction(outputs_decoder_1_3_1, torch.unsqueeze(inputs[1], 2))

            self.encoder.zero_grad()
            self.classifier.zero_grad()
            self.discriminator.zero_grad()

            loss_discriminator_3_1.backward(retain_graph=True)
            loss_classifier_3_1.backward(retain_graph=True)
            ((loss_reconstruction_0+loss_reconstruction_1)/2).backward()
        ...
        return accuracy_3_1, accuracy_3_2, avg_loss_3_1, avg_loss_3_2,conf_matrix_3_1, conf_matrix_3_2
    ...
    def train(self, epochs, dataset_type):
        ...
        for a in range(epochs):
            accuracy_3_1, accuracy_3_2, avg_loss_3_1, avg_loss_3_2,conf_matrix_3_1, conf_matrix_3_2= self.train_one_epoch()
...
if __name__ == "__main__":
    ...
    path_encoder = f"./models/{input_parameters['dataset']}/{input_parameters['experiment']}/{input_parameters['seed']}/step_2/encoder.pt"
    path_decoder = f"./models/{input_parameters['dataset']}/{input_parameters['experiment']}/{input_parameters['seed']}/step_2/decoder.pt"
    path_discriminator = f"./models/{input_parameters['dataset']}/{input_parameters['experiment']}/{input_parameters['seed']}/step_2/discriminator.pt"
    path_classifier = f"./models/{input_parameters['dataset']}/{input_parameters['experiment']}/{input_parameters['seed']}/step_2/classifier.pt"
    ...
    new_disc = Discriminator_1()
    # Adjusting state dict to match the new model keys, if necessary
    new_state_dict = new_disc.state_dict()
    for name, param in discriminator_state_dict.items():
        if name in new_state_dict and param.size() == new_state_dict[name].size():
            new_state_dict[name].copy_(param)
        else:
            print(f"Skipping {name} due to size mismatch or new layer.")

    new_disc.load_state_dict(new_state_dict, strict=False)
    ...
    train_3 = Trainer_3(encoder, decoder, new_disc, classifier, discriminator_train_dataloader, 
    ...
    path = f'./models/{input_parameters["dataset"]}/{input_parameters["experiment"]}/{input_parameters["seed"]}/step_3/'
