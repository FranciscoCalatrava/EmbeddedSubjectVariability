...
class Trainer_3():
    def __init__(self, encoder, decoder, discriminator, classifier, train_dataloader_discriminator, val_dataloader_discriminator, train_dataloader_classifier, val_dataloader_classifier, test_dataloader, device, optimizer, criterion, weight, dataset, experiment, seed, id) -> None:
        self.encoder = encoder.to(device)
        self.decoder = decoder.to(device)
        self.classifier = classifier.to(device)
        self.discriminator = discriminator.to(device)

        self.train_dataloader_discriminator = train_dataloader_discriminator
        ...
        self.test_dataloader_classifier = test_dataloader
        ...
        self.seed = seed
        self.id = id
        self.window = {'PAMAP2':512,'REALDISP':256, 'MHEALTH':512}
        self.sensor = {'PAMAP2':18,'REALDISP':54, 'MHEALTH':15}
    ...
    def train_one_epoch(self):
        ...
        loss_discriminator, loss_feature_extractor = [],[]


        
        for batch_discriminator, batch_classifier in zip(self.train_dataloader_discriminator, self.train_dataloader_classifier):
            # self.encoder.train(True)
            # self.decoder.eval()
            # self.classifier.train(True)
            ...
            inputs,targets = batch_discriminator
            ...
            assert inputs[0].shape[1] == self.sensor[self.dataset]
            assert inputs[0].shape[2] == self.window[self.dataset]
    
            assert inputs[1].shape[1] == self.sensor[self.dataset]
            assert inputs[1].shape[2] == self.window[self.dataset]

            inputs_classifier, targets_classifier = batch_classifier[0].to(self.device).float(), batch_classifier[1].to(self.device).long()

            assert inputs_classifier.shape[1] == self.sensor[self.dataset]
            assert inputs_classifier.shape[2] == self.window[self.dataset]
            ...
            loss_feature_extractor.append(loss.item())
        ...
        return accuracy_3_1, accuracy_3_2, avg_loss_3_1, avg_loss_3_2,conf_matrix_3_1, conf_matrix_3_2, loss_discriminator, loss_feature_extractor
    ...
    def test_classifier(self):
        ...
        with torch.no_grad():
            for classifier_batch in self.test_dataloader_classifier:
                inputs_classifier, targets_classifier = classifier_batch[0].to(self.device).float(), classifier_batch[1].to(self.device)
    ...
    def train(self, epochs, dataset_type):
        #wandb.init(project='Percom', name=f"{dataset_type}_{self.experiment}_{self.seed}_ID{self.id}")
        threshold = 0.5
        threshold_dict = {"PAMAP2": 0.92, "MHEALTH":0.96, "REALDISP":0.95}
        ...
        max_value = 0.0
        index_list = []
        scheduler_discriminator = optim.lr_scheduler.StepLR(self.optimizer_discriminator, step_size =10 , gamma = 0.4, verbose = True)
        scheduler_encoder = optim.lr_scheduler.StepLR(self.optimizer_encoder, step_size = 40, gamma = 0.4, verbose = True)
        scheduler_decoder = optim.lr_scheduler.StepLR(self.optimizer_classifier, step_size = 40 , gamma = 0.4, verbose = True)
        # scheduler_discriminator = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer_discriminator, 'min', factor = 0.6, verbose = True)
        # scheduler_encoder = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer_encoder,'min', factor = 0.6, verbose = True)
        # scheduler_decoder = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer_classifier, 'min', factor = 0.6, verbose = True)
        early_stopping_classifier = EarlyStopping(patience=4, verbose=True, path='model_checkpoint.pt')
        early_stopping_encoder = EarlyStopping(patience=4, verbose=True, path='model_checkpoint.pt')
        for a in range(epochs):
            accuracy_3_1, accuracy_3_2, avg_loss_3_1, avg_loss_3_2,conf_matrix_3_1, conf_matrix_3_2, loss_discriminator, loss_feature_extractor= self.train_one_epoch()

            #for c,d in zip(loss_discriminator,loss_feature_extractor):
                #wandb.log({"Discriminator_loss": c, "feature_extractor_loss": d})
            ...
            if a%1 == 0:
                ...
                accuracy_classifier_test, avg_loss_classifier_test, conf_matrix_classifier_test = self.test_classifier()
                ...
                print(f"|TEST| Acc Classifier {accuracy_classifier_test['classifier']}")
                ...
                if len(buffer_values) > 80 and accuracy_classifier['classifier'] > threshold and accuracy_discriminator['discriminator'] < 0.6:
                    early_stopping_classifier(accuracy_classifier['classifier'], self.classifier)
                    early_stopping_encoder(accuracy_classifier['classifier'], self.encoder)
            ...
            print(self.optimizer_encoder.param_groups[0]['lr'])
            print(self.optimizer_classifier.param_groups[0]['lr'])
            print(self.optimizer_discriminator.param_groups[0]['lr'])
        ...
        self.encoder = early_stopping_encoder.best_model
        self.classifier = early_stopping_classifier.best_model
        accuracy_classifier_test, avg_loss_classifier_test, conf_matrix_classifier_test = self.test_classifier()
        print(f"|TEST| Acc Classifier {accuracy_classifier_test['classifier']}")
        # wandb.finish()
...
def get_discriminator_dataset(path,trainsize):
    ...
    with h5py.File(path+f"train_{trainsize}.h5",'r') as file:
        # print("Keys: %s" % list(file.keys()))
    ...
    with h5py.File(path + f"validation_{trainsize}.h5",'r') as file:
        # print("Keys: %s" % list(file.keys()))
...
def get_dataset_3(dataset, experiment, bs,PATH, trainsize):
...
if __name__ == "__main__":
    ...
    path_encoder = MLSP_DATA_ROOT + f"/models/MLSP2025/{input_parameters['dataset']}/{input_parameters['experiment']}/{input_parameters['seed']}/step_2/encoder.pt"
    path_decoder = MLSP_DATA_ROOT + f"/models/MLSP2025/{input_parameters['dataset']}/{input_parameters['experiment']}/{input_parameters['seed']}/step_2/decoder.pt"
    path_discriminator = MLSP_DATA_ROOT + f"/models/MLSP2025/{input_parameters['dataset']}/{input_parameters['experiment']}/{input_parameters['seed']}/step_2/discriminator.pt"
    path_classifier = MLSP_DATA_ROOT + f"/models/MLSP2025/{input_parameters['dataset']}/{input_parameters['experiment']}/{input_parameters['seed']}/step_2/classifier.pt"
    ...
    path = MLSP_DATA_ROOT + f'/models/MLSP2025/{input_parameters["dataset"]}/{input_parameters["experiment"]}/{input_parameters["seed"]}/step_3/'
