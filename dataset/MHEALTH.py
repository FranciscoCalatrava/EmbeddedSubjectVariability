...
class MHEALTH():
    ...
    def data_segmentation(self):
        train_data_segmented = self.segment_data(self.training_cleaned, 128, 64)
        validation_data_segmented = self.segment_data(self.validation_cleaned, 128, 64)
        test_data_segmented = self.segment_data(self.test_cleaned, 128, 64)
    ...
    def prepare_dataset(self):
        ...
        for a in self.training_normalized_segmented.keys():
            for b in self.training_normalized_segmented[a]:
                training.append((np.transpose(b.iloc[:, 3:-1].to_numpy()), int(b.iloc[0, -1] - 1), int(a)))
        for a in self.validation_normalized_segmented.keys():
            for b in self.validation_normalized_segmented[a]:
                validation.append((np.transpose(b.iloc[:, 3:-1].to_numpy()), int(b.iloc[0, -1] - 1), int(a)))
        for a in self.test_normalized_segmented.keys():
            for b in self.test_normalized_segmented[a]:
                testing.append((np.transpose(b.iloc[:, 3:-1].to_numpy()), int(b.iloc[0, -1]) - 1, int(a)))
