...
def transform_into_list_discriminator(outputs, labels):
    tensor = outputs.detach()
    data = [(a,b) for a,b in zip(tensor,labels)]
    return data

def save_latent_space_discriminator(dataset, experiment, seed, data, epoch, step, model,name,id):
    ...
    with h5py.File(path+f'{name}_latent_space.h5', 'w') as hf:
                for i, (data, disc_label) in enumerate(data):
                    grp = hf.create_group(f'item_{i}')
                    grp.create_dataset('data', data=data.cpu().numpy())
                    grp.create_dataset('disc_label', data=disc_label.cpu().numpy())
